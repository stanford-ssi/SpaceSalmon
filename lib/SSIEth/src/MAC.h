...
private:
	...
	COMPILER_ALIGNED(8)
	struct _mac_txbuf_descriptor _txbuf_descrs[CONF_GMAC_TXDESCR_NUM];

	COMPILER_ALIGNED(8)
	struct _mac_rxbuf_descriptor _rxbuf_descrs[CONF_GMAC_RXDESCR_NUM];

	COMPILER_ALIGNED(32)
	uint8_t _rxbuf[CONF_GMAC_RXDESCR_NUM][CONF_GMAC_RXBUF_SIZE];

	struct pbuf *tx_pbuf[CONF_GMAC_TXDESCR_NUM];
	...
	volatile uint32_t _txbuf_index;
	volatile uint32_t _last_txbuf_index;
	volatile uint32_t _rxbuf_index;

	result_t _init_bufdescr();

public:
	const Gmac *hw; /*!< Hardware module instance handler */
	MAC(const Gmac *_hw) : hw(_hw){};
	result_t init();
	result_t deinit();
	result_t enable();
	result_t disable();
