...
uint32_t toneMaxFrequency = CONF_CPU_FREQUENCY / 2;
...
{
  //oneMaxFrequency = accurateSystemCoreClockFrequency / 2;
}
...
void tone (uint32_t outputPin, uint32_t frequency, uint32_t duration)
{
  ...
  portToggleRegister = &(PORT->Group[GPIO_PORT(outputPin)].OUTTGL.reg);
  portClearRegister = &(PORT->Group[GPIO_PORT(outputPin)].OUTCLR.reg);
  portBitMask = (1ul << GPIO_PIN(outputPin));
  ...
  if (outputPin != lastOutputPin)
  {
    ...
    gpio_set_pin_level(outputPin, false);
    gpio_set_pin_direction(outputPin, GPIO_DIRECTION_OUT);
  }
  toneIsActive = true;
