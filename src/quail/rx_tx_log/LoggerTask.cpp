...
void LoggerTask::findFile(char* filename, size_t filesize, int* lognum) {
    for(FRESULT res = FR_OK; res != FR_NO_FILE; (*lognum)++) {
        ...
        if (res != FR_OK) {
            sys.telem_slate.error.set(ErrorType::corrupt_file);
        }
    }
}
...
void LoggerTask::activity()
{
    ...
    while (res != FR_OK) {
        ...
        sys.telem_slate.error.set(ErrorType::failed_to_mount_disk);
        memset(&fs, 0, sizeof(FATFS));
        res = f_mount(&fs, "", 1);
        vTaskDelay(5000);
    } 
    ...
    do {
        ...
        sys.telem_slate.error.set(ErrorType::failed_to_open_file);
